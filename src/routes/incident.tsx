    ...
    <div>
      ...
      <div className="mb-4 flex gap-1">
        {Array.from({ length: totalSteps }).map((_, i) => (
          <div key={i} className={`h-1 flex-1 ${i + 1 <= step ? "bg-primary" : "bg-border"}`} />
        ))}
      </div>
      ...
      <div className="bg-card border border-border p-6 space-y-4">
        {step === 1 && (
          <>
            ...
            <Field label={t.incident.caseNumber}><input className={inputCls} value={data.caseNumber} onChange={(e) => update("caseNumber", e.target.value)} /></Field>
            <Field label={t.incident.dateTime}><input type="datetime-local" className={inputCls} value={data.dateTime} onChange={(e) => update("dateTime", e.target.value)} /></Field>
            <Field label={t.incident.location}>
              <div className="flex gap-2">
                ...
                <button type="button" className="px-3 py-2 text-xs bg-secondary border border-border whitespace-nowrap"
                  onClick={() => {
                    if (!navigator.geolocation) return;
                    navigator.geolocation.getCurrentPosition((p) =>
                      update("location", `${p.coords.latitude.toFixed(5)}, ${p.coords.longitude.toFixed(5)}`));
                  }}>{t.incident.useGps}</button>
              </div>
            </Field>
            <Field label={t.incident.borderPoint}><input className={inputCls} value={data.borderPoint} onChange={(e) => update("borderPoint", e.target.value)} /></Field>
          </>
        )}
        {step === 2 && (
          <>
            <h2 className="font-semibold text-lg">{t.incident.officer}</h2>
            <Field label={t.incident.officerName}><input className={inputCls} value={data.officerName} onChange={(e) => update("officerName", e.target.value)} /></Field>
            <Field label={t.incident.badgeId}><input className={inputCls} value={data.badgeId} onChange={(e) => update("badgeId", e.target.value)} /></Field>
            ...
            <Field label={t.incident.witnessName}><input className={inputCls} value={data.witnessName} onChange={(e) => update("witnessName", e.target.value)} /></Field>
            <Field label={t.incident.witnessId}><input className={inputCls} value={data.witnessId} onChange={(e) => update("witnessId", e.target.value)} /></Field>
          </>
        )}
        {step === 3 && (
          <>
            ...
            <Field label={t.incident.deviceType}>
              <select className={inputCls} value={data.deviceType} onChange={(e) => update("deviceType", e.target.value)}>
                {Object.entries(t.deviceTypes).map(([k, v]) => (<option key={k} value={k}>{v}</option>))}
              </select>
            </Field>
            <div className="grid grid-cols-2 gap-3">
              <Field label={t.incident.make}><input className={inputCls} value={data.make} onChange={(e) => update("make", e.target.value)} /></Field>
              <Field label={t.incident.model}><input className={inputCls} value={data.model} onChange={(e) => update("model", e.target.value)} /></Field>
              <Field label={t.incident.serial}><input className={inputCls} value={data.serial} onChange={(e) => update("serial", e.target.value)} /></Field>
              <Field label={t.incident.imei}><input className={inputCls} value={data.imei} onChange={(e) => update("imei", e.target.value)} /></Field>
            </div>
            <Field label={t.incident.condition}><textarea className={inputCls} rows={2} value={data.condition} onChange={(e) => update("condition", e.target.value)} /></Field>
          </>
        )}
        {step === 4 && (
          <>
            ...
            <Field label={t.incident.network}><input className={inputCls} value={data.network} onChange={(e) => update("network", e.target.value)} /></Field>
          </>
        )}
        {step === 5 && (
          <>
            <h2 className="font-semibold text-lg">{t.incident.circumstances}</h2>
            <Field label={t.incident.circumstancesDesc}><textarea className={inputCls} rows={5} value={data.circumstances} onChange={(e) => update("circumstances", e.target.value)} /></Field>
      ...
      <div className="mt-4 flex justify-between">
        <button className="px-4 py-2 text-sm border border-border bg-background disabled:opacity-50"
          onClick={() => setStep((s) => Math.max(1, s - 1))} disabled={step === 1}>{t.common.back}</button>
        {step < totalSteps ? (
          <button className="px-4 py-2 text-sm bg-primary text-primary-foreground" onClick={() => setStep((s) => s + 1)}>{t.common.next}</button>
        ) : (
          <div className="flex gap-2">
            <button className="px-4 py-2 text-sm border border-border"
              onClick={() => exportIncidentPdf({ ...data, id: evidenceId, createdAt: new Date().toISOString() })}>
              {t.common.exportPdf}
            </button>
            <button className="px-4 py-2 text-sm bg-primary text-primary-foreground disabled:opacity-50" onClick={handleSave} disabled={saving}>
